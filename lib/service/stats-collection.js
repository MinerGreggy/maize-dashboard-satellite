...
class StatsCollection {
  ...
  async init() {
    ...
    this.origin = 'maize-dashboard-satellite';
    ...
    this.connection.addService(constants.SERVICE('maize').walletUi);
  ...
  async updateRunningServices() {
    ...
    await Promise.all(this.enabledServices.filter(service => service !== plotterService).map(async service => {
      let isRunning = await this.daemonApiClient.isServiceRunning(constants.SERVICE('maize')[service]);
